...
This results in a well-defined~{\action{$S_n$}} on~$V^{\tensor n}$ via homomorphisms of {\dgvs}, given by
\begin{gather*}
...
A map~$f \colon V \to W$ is \defemph{graded} of \defemph{degree}~$d = \hdeg{f}$ if~$f(V_n) \subseteq V_{n+d}$ for all~$n$, and~$\dgHom(V,W)$ is the {\dgv} with
\begin{align*}
  \dgHom(V,W)_n
  &=
  \{
...
\begin{definition}
  ...
  \begin{enumerate}
    \item
      If the conditions of \cref{characterization of bialgebras} are satisfied then~$(B, \mu, u, \Delta, \varepsilon)$ is a \defemph{\dgb}.
    \item
...
\begin{remark}
  ...
  The antipode of a {\dgh}~$H$ is the the unique homomorphism of {\dgvs}~$S \colon H \to H$ that makes the diagram
  \begin{equation}
...
\begin{example}
  ...
  \begin{align*}
    ...
    v_n \dotsm v_1
  \end{align*}
  for homogeneous~$v_i$, where~$S$ is viewed as a map~$\dgTensor(V) \to \dgTensor(V)$ and~$\Sh(p,q)$ denotes the set of~{\shuffles{$p$}{$q$}}.
...
\begin{definition}
  The \defemph{universal enveloping algebra} of a {\dgl}~$\glie$ is
  \[
    \Univ(\glie)
    =
    \dgTensor(\glie)
