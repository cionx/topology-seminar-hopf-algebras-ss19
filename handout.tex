...
\[
  d(v \tensor w)
  =
  d(v) \tensor w + (-1)^{\hdeg{d} \hdeg{v}} v \tensor d(w)
  =
  d(v) \tensor w + (-1)^{\hdeg{v}} v \tensor d(w) \,.
\]
Higher tensor products~$V_1 \tensor \dotsb \tensor V_n$ are defined inductively.
  ...
  \begin{enumerate}
    ...
    \item
      ...
      \[
        ...
        v_1 \dotsm v_n
        \mapsto
        \frac{1}{n!} \sum_{\sigma \in S_n} (-1)^{n(\sigma)}
        v_{\sigma(1)} \tensor \dotsb \tensor v_{\sigma(n)}
      \]
      with~$n(\sigma) = \sum \{ \hdeg{v_i} \hdeg{v_j} \suchthat 1 \leq i < j \leq n, \sigma(i) > \sigma(j) \}$ is a section for the projection~$p \colon \dgTensor(V) \to \dgSymm(V)$.
...
%     TODO: Check compatiblity with differential.
      \[
        \begin{tikzcd}[column sep = huge, row sep = 4em]
          ...
          \arrow[bend right = 15]{d}[left]{p_n}
          \\
          \homology_n(\dgSymm(V))
          ...
          \arrow[bend right = 15]{u}[right]{s_n}
      ...
      We have~$\beta_n = p_n \circ \alpha_n \circ \homology_n(s)$.
      The composition~$\beta'_n \defined \homology_n(p) \circ \alpha_n^{-1} \circ s_n$ is given by
      \[
        \beta'_n([v_1] \dotsm [v_n])
        =
        [v_1 \dotsm v_n]
      \]
      and hence an inverse to~$\beta_n$.
...
For this we need a version of the Poincaré–Birkhoff–Witt theorem (PBW~theorem) for {\dgl};
we will not prove this, but refer to~\cite[Appendix~B,Theorem~2.3]{quillen} and~\cite[\S21(a)]{rational_homotopy_book}
...
\begin{proof}
  We denote the isomorphisms~$\dgSymm(\glie) \to \Univ(\glie)$ and~$\dgSymm(\homology(\glie)) \to \Univ(\homology(\glie))$ from \cref{consequences of pbw} by~$e$.
...
If~$C$ is any other {\dga} and if~$f \colon A \to C$ and~$g \colon B \to C$ are two morphisms of {\dgas} with
\[
  f(a) g(b)
  =
