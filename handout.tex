...
\titlehead{Graduate Seminar on Topology \hfill May 20, 2019}
...
A \defemph{graded vector space} is a vector space~$V$ together with a grading~$V = \bigoplus_{n \in \Integer} V_n$.
...
\end{center}
A map~$f \colon V \to W$ between graded vector spaces is \defemph{graded} of \defemph{degree}~$\hdeg{f} = d$ if~$f(V_n) \subseteq V_{n+d}$ for all~$n$.
...
\begin{center}
  \fbox{graded $\iff$ differential graded with~$d = 0$}
\end{center}
...
\begin{center}
  \fbox{Whenever elements~$x$,~$y$ are swapped, the sign~$(-1)^{\hdeg{x} \hdeg{y}}$ is introduced.}
\end{center}
...
If~$V$,~$W$ are {\dgvs} then the \defemph{algebraic Künneth isomorphism} is the natural isomorphism of graded vector spaces
\[
  \homology(V \tensor W)
  \cong
...
\begin{definition}
  A \defemph{differential graded algebra} or \defemph{\dga} is a {\dgv}~$A$ together with morphisms of {\dgvs}~$m \colon A \tensor A \to A$ and~$u \colon k \to A$ that make the diagrams
  \[
...
\begin{definition}
  If~$A$ is a {\dga} then the \defemph{{\dgcom}} is given by
  \[
    [a,b]
    \defined
...
%       The antipode is a morphism of bialgebras~$H \to H^{\op,\cop}$.
  \end{enumerate}
\end{remark}%
% TODO: Check that the antipode is an antimorphism.
...
\begin{definition}
  A \defemph{\dgl} is a {\dgv}~$\glie$ together with a morphism~$[-,-] \colon \glie \tensor \glie \to \glie$ such that~$[-,-]$ is \defemph{graded skew symmmetric} in the sense that the diagram
  \[
    \begin{tikzcd}[column sep = small]
      \glie \tensor \glie
      \arrow{rr}[above]{\tau}
...
\begin{remark}
  ...
  \shortintertext{and}
    ...
    \notag
  \end{align}
  Condition \eqref{pre dgjacobi} can be rewritten by the graded skew-symmetry of~$[-,-]$ as the \defemph{graded Jacobi identity}
  \[
    \sum_{\text{cyclic}}
...
\begin{definition}
  The \defemph{universal enveloping algebra} of a {\dgl}~$\glie$ is a {\dga}~$\Univ(\glie)$ together with a morphism of~{\dgls}~$i \colon \glie \to \Univ(\glie)$ such that for every other {\dga}~$A$ and every morphism of~{\dgls}~$f \colon \glie \to A$ there exists a unique morphism of {\dgas}~$F \colon \Univ(\glie) \to A$ that extends~$f$, i.e.\ that makes the following diagram commute:
  \[
    \begin{tikzcd}
      \Univ(\glie)
      \arrow[dashed]{r}[above]{F}
